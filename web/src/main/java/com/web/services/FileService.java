...
public class FileService {
    ...
    public void saveFile(List<MultipartFile> files, Product product) throws FileException, IOException {
        for(MultipartFile f : files) {
            String originalFileName = f.getOriginalFilename();
            byte[] fileData = f.getBytes();

            if (originalFileName != null) {
                String[] extensions = originalFileName.split("\\.");
                UUID filename = UUID.randomUUID();
                String strFilename = filename.toString() + "." + extensions[extensions.length - 1];
                String path = imagePath + SEP + strFilename;
                saveByteData(fileData, path);

                Image image = new Image();
                image.setAltName(originalFileName);
                image.setPath(path);
